...
\begin{document}
    ...
    \begin{abstract}
        Solving the Traveling Salesman Problem (TSP)enables buissness to optimize logistics and route planning. This is the case, for example, for companies like Amazon who may wish to maximize the efficieny and speed of their deliver services. We developed two algorithms, namely \emph{opt2} and \emph{greedy}, to address the TSP in the context of package delivery companies (like Amazon, USP, USPS, etc). We then used these algorithms to build a product protoype these companies could use to schedule routes. Furthermore, we explored two different scenarios of our algorithms. We looked at how varying the distance limit and number of trucks deployed would affect the number of unfulfilled orders our drivers can take. We discovered that having a distance limit of thirty-four units was sufficient to prevent an unrecoverable number of unfullfilled orders. Secondly, at a distance of 34 units, we determined that about three truck drivers were sufficient to ensure a steady number of unfulfilled orders (under 20 orders), thus improving delivery times. Lastly, we observed the efficiency of our simulation between our local computer and the TACC supercomputers.
    \end{abstract}
